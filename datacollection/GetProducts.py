...
#Product Name,Vendor Name,Product Type,Number of Vulnerabilities,Vendor ID,Product ID,Sha

#&version_id=&page=1&hasexp=0&opdos=0&opec=0&opov=0&opcsrf=0&opgpriv=0&opsqli=0&opxss=0&opdirt=0&opmemc=0&ophttprs=0&opbyp=0&opfileinc=0&opginf=0&cvssscoremin=0&cvssscoremax=0&year=0&month=0&cweid=0&order=1&trc=2357&sha=544260ec3a86a7e17f8b02b39d6342815d8d4bd5
def getProductSpecificCVEs(product,vendor,ProductType,noOfVulnerabilities,vendorID,productID,sha):
    ...
    for index in range(1,pages):   
        ...
        url = 'https://www.cvedetails.com/vulnerability-list.php?vendor_id='+ str(vendorID) + '&product_id=' + str(productID) + '&version_id=&page='+ str(index) + url2 + str(noOfVulnerabilities) + '&sha=' + sha
        #print(url)
    ...
    bigframe.to_csv('.\\ProductMapping\\'+vendor+'_'+product+'.csv') 

def concatenateProducts():
    path ="D:\\repos\\APDSProject\\MLTuning\\datacollection\\ProductMapping"
    ...
    biggest_frame.to_csv('product_cve_map.csv') 


def main():
    print("Collecting the Vendors")

    if not os.path.exists('ProductMapping'):
        os.makedirs('ProductMapping')

    #Loop through the top 50 vendors
    vendors = pd.read_csv("D:\\repos\\APDSProject\\MLTuning\\modeling\\products.csv")
    #vendors.info()
    for index, row in vendors.head(n=50).iterrows():
         print(index, row['Product Name'], row['Vendor Name'], row['Number of Vulnerabilities'],row['Vendor ID'], row['Sha'])
         getVendorSpecificCVEs(row['Product Name'],row['Vendor Name'], row['Product Type'],row['Number of Vulnerabilities'],row['Vendor ID'], row['Product ID'],row['Sha'])
